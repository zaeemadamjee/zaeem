...
description: "Keep a reviewable decision trail for long-running or unattended work: a TSV log with one row per decision (what, why, evidence, result). Local by default; commit it when a reviewer needs the trail to trust the result. Use for /show-me-your-work, autonomous or multi-phase runs, or work a human reviews after stepping away. Use show-me instead for visual explanations."
---
...
At the end of the run, before handing back, check the log told the truth. Read this run's transcript if your harness persists one (stay inside the active workspace's sessions; don't read transcripts from unrelated projects), or replay your own message history if it doesn't. Walk the log against what actually happened:
...
Before handing back, spawn a review subagent, on a different model family from the one that did the work when your harness supports model selection. Self-review is not a substitute; the point is fresh eyes you cannot bring yourself. If your harness has no subagents, do the scan yourself in a separate pass and label it as a self-review in the reply. The subagent reads the audit trail and the run's transcript, then flags what the user should pay attention to. Not a redo of the work, a scan for what's suboptimal or risky.
