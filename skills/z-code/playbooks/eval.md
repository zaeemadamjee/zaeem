...
6. **Verify the chain from transcripts, not self-report.** Read each candidate's transcript if your harness persists them (stay inside the active workspace's sessions; do not read transcripts from unrelated projects). Look at which files each candidate actually opened. Citing a principle is not reading its leaf skill, and reading it is not applying it. Grade chain-following from the files it really read plus the shape of the code, never from the candidate's own claims.
