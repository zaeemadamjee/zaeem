...
2. Pick the wake mechanism using whatever looping or scheduled re-invocation your harness offers (a loop command, a watcher process, or simply continuing until done). An event to watch (CI, a merge, a ref advancing) gets a watcher subagent that wakes you on the event, with a long time-based heartbeat as fallback. No event gets a fixed-interval heartbeat sized to when the result is worth re-checking.
...
4. Mid-run discoveries are yours. Address broken skills, related bugs, flaky verifiers, review noise, tooling failures, orphaned follow-ups, and fixable drift yourself via z-code. Put out-of-band fixes in their own PR. Do not park reversible work for the human or stop to ask about it. Surface only irreversible actions, genuine product or preference calls no experiment can settle, or a real dead end. Keep the predicate as the main drive, and return to it after each side fix.
