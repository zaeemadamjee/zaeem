...
- **Worker / verifier.** Prefer remote/background agents when your harness offers them, unless the task needs this machine: live UI or CLI runtime verification against local state; reading local session transcripts; simulators and local IDE state; auth that exists only here. Remote agents cannot read the local store, so their briefs inline what they need or point at repo paths. Prefer fewer, broader workers; one writer per worktree or branch (principle-separate-before-serializing-shared-state), laid out per `../references/worktrees.md`. Run a unit's verifier on a different model family from its worker.
...
VERIFY       exact commands or the runtime-verification steps, plus known gotchas
...
- The frontier is a computed object, never narrative. Recompute `frontier.json` from the stacking tool (`../references/stacking.md`) after every merge and stack mutation because GitHub base refs drift mid-restack while the tool's tracking is authoritative: ordered PR list, branch names, head SHAs, a generation number, the lowest unmerged PR. Resolve it where the tool knows the stack, normally the stacker's clone; a checkout whose stack metadata never saw the submits reports no PRs and the command errors rather than guessing.
- Exactly one stacker per stack may run stack-mutating commands, serialized within its stack; record the holder in the standing orders. Restacks run remotely when possible; a local restack at this scale takes the laptop down.
- Workers never rebase and never run stack-mutating commands. Babysitters follow `playbooks/babysit.md`, one per stack, scoped to one immutable frontier generation; they report conflicts to the stacker rather than restacking.
...
- Never resume an agent to check on it; a resume restarts an idle agent. Probe read-only: the ledger, `units.tsv`, `gh`, pushed branches, the remote agent's status in your harness's dashboard. Transcript mtime is not liveness.
