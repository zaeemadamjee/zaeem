...
3. Migrate one component at a time. Each is an independent artifact, so parallelize across worktrees, one owner per component (the **separate-before-serializing-shared-state** principle skill; layout and creation in `../references/worktrees.md`). Shared primitives migrate first as a blocking phase.
