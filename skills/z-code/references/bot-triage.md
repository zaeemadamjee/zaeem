...
Use this reference when the Babysit playbook (`../playbooks/babysit.md`) handles automated review comments (Bugbot, Copilot review, security bots, and similar). The goal is not to ignore the bot by default. The goal is to stop treating every comment as a required code change.
...
- Confidence: candidate
- Skip when: The bot flags an export, component, helper, or file as unused, and the stack topology (`gh stack view` or `gt ls -s`), upper-stack diffs, or PR context shows it is used by a later PR in the stack.
...
- Source: one prose-pinning PR with eight bot passes; the claim was real on
