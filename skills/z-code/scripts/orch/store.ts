...
function ghStackViewJson(repo: string): string | null {
  try {
...
function ghStackPullRequest({
  ...
  try {
    ...
    throw new UserError(
      `gh pr view ${branch} failed; the branch may have no pull request yet, so resolve the frontier after gh stack submit: ${errorMessage(error)}`
    );
  }
  let parsed: unknown;
  try {
    ...
    throw new UserError(`gh pr view ${branch} returned invalid JSON`);
  }
  if (
    ...
    throw new UserError(`gh pr view ${branch} returned an unexpected shape`);
  }
  const pr = (parsed as { number: unknown }).number;
  const state = (parsed as { state: unknown }).state;
  if (typeof pr !== "number" || !Number.isSafeInteger(pr) || pr <= 0) {
    throw new UserError(`gh pr view ${branch} returned an invalid PR number`);
  }
  if (state !== "OPEN" && state !== "CLOSED" && state !== "MERGED") {
    throw new UserError(
      `gh pr view ${branch} returned an unknown PR state: ${String(state)}`
    );
  }
  return { pr, state };
}

function ghStackFrontier({
  raw,
  repo,
}: {
  raw: string;
  ...
  try {
    ...
    throw new UserError("gh stack view --json returned invalid JSON");
  }
  if (
    typeof parsed !== "object" ||
    parsed === null ||
    ...
    throw new UserError("gh stack view --json returned an unexpected shape");
  }
  const branches: string[] = [];
  for (const node of (parsed as { branches: unknown[] }).branches) {
    if (
      typeof node !== "object" ||
      node === null ||
      !("name" in node) ||
    ...
    const branch = (node as { name: string }).name;
    if (branches.includes(branch)) {
      throw new UserError(
        `gh stack view --json contains duplicate branch ${branch}`
      );
    }
    branches.push(branch);
  }
  ...
  const result = branches.map((branch) => ({
    branches: branch,
    ...ghStackPullRequest({ branch, repo }),
  }));
  if (new Set(result.map((row) => row.pr)).size !== result.length) {
    throw new UserError(
      "gh stack view --json resolved duplicate pull requests"
    );
  }
  return result;
}

function stackFrontier(repo: string): readonly GtFrontierEntry[] {
  const ghStackRaw = ghStackViewJson(repo);
  if (ghStackRaw !== null) {
    return ghStackFrontier({ raw: ghStackRaw, repo });
  }
  return graphiteFrontier(repo);
}
