...
`
  ...
  await writeFile(
    gh,
    `#!/usr/bin/env bash
printf 'no gh-stack in this fixture\\n' >&2
exit 2
`
  );
...
`;
  ...
  it("resolves the frontier through gh stack when available", async () => {
    const { directory, store } = await initializedStore();
    const stack = await makeGitStack(directory);
    const bin = join(directory, "bin");
    await mkdir(bin);
    ...
    await writeFile(
      ...
      `#!/usr/bin/env bash
set -euo pipefail
if [ "$(pwd -P)" != "${realpathSync(stack.repo)}" ]; then
  printf 'gh ran outside the fixture repo: %s\\n' "$(pwd -P)" >&2
  exit 2
fi
case "$*" in
  "stack view --json")
    printf '{"trunk":"main","currentBranch":"stack/open","branches":[{"name":"stack/merged","isMerged":true},{"name":"stack/closed","isMerged":false},{"name":"stack/open","isMerged":false}]}\\n'
    ;;
  "pr view stack/merged --json number,state")
    printf '{"number":10,"state":"MERGED"}\\n'
    ;;
  "pr view stack/closed --json number,state")
    printf '{"number":13,"state":"CLOSED"}\\n'
    ;;
  "pr view stack/open --json number,state")
...
`
    ...
    await writeFile(
      gt,
      `#!/usr/bin/env bash
printf 'gt must not run when gh stack resolves the frontier\\n' >&2
exit 2
`
    );
    await chmod(gt, 0o755);
