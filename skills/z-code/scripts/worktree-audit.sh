...
# Session-transcripts dir, if the agent harness persists one for this repo.
# Override with WORKTREE_AUDIT_TRANSCRIPTS; the LAST_CHAT column degrades to
# "-" when no transcript dir exists. Known layouts probed below:
#   Cursor:   ~/.cursor/projects/<slugified-repo-path>/agent-transcripts
#   opencode: ~/.local/share/opencode/project/<slugified-repo-path>/storage
slug=$(printf '%s' "$main_wt" | sed 's#^/##; s#/#-#g')
transcripts="${WORKTREE_AUDIT_TRANSCRIPTS:-}"
if [ -z "$transcripts" ]; then
	for candidate in \
		"$HOME/.cursor/projects/$slug/agent-transcripts" \
		"$HOME/.local/share/opencode/project/$slug/storage"; do
		[ -d "$candidate" ] && transcripts="$candidate" && break
	done
fi
now=$(date +%s)
