---
name: z-code
description: Agent style for concise, detailed responses, deliberate subagents, unslopped prose, simple code, and verified work. Use for z-code, /z-code, or requests to work in this style.
---
...
- An automated reviewer (Bugbot, Copilot review, a security-review bot) commented → skeptical posture. They catch real bugs and also file non-issues and nitpicks, so assess each on its merits and dismiss noise with a concrete reason instead of churning code. Triage fix / dismiss / ask per `references/bot-triage.md`.
...
**Every subagent you spawn inside a playbook step** (code-writing delegates, ad-hoc helpers) gets instructed to read this skill's SKILL.md in full before working, so it operates in the same style. Routed workflow skills (`how`, `why`, `interrogate`, `reflect`, `swarm`) prescribe their own subagent shapes for diverse-model review; respect what the skill prescribes.

**Defaults for every subagent call.** Run in the background when your harness supports it, keep MCP/tool access when the subagent needs it (some readonly/plan modes strip MCP), pass file pointers not inlined context. When your harness supports per-subagent model selection, tier code delegates by difficulty: the hardest changes (cross-cutting design, gnarly concurrency, subtle algorithms) go to your strongest judgment model when the task needs judgment or the intent is vague, and to your strongest instruction-following model when the work is a precisely specified sequence of steps to execute to the letter; trivial mechanical edits go to a fast code model. Without subagent support, do the work inline and keep this skill's discipline; the delegation structure is an optimization, not the contract.
...
- **Shipping.** The half after Babysit. Independently verifying a green stack, then landing the contiguous verified run with your stacking tool's merge command (`references/stacking.md`). `playbooks/shipping.md`.
