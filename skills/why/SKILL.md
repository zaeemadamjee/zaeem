...
Before spawning investigators, list the MCP servers and external tools available in your environment (however your harness exposes them: a tool list, an MCP config, or the tools already in your context).
...
Subagent config (each): a subagent with MCP/tool access. If your harness's readonly or plan mode strips MCP access, don't use it here; it disables MCP-backed investigators entirely. Investigators still shouldn't write anything. That's a posture, not a sandbox. If your harness doesn't support subagents, run the categories yourself sequentially, one category at a time.
...
Spawn one synthesizer subagent (strongest reasoning model if your harness supports model selection; yourself if subagents are unavailable). The synthesizer's quality check spot-verifies citations, which can require MCP access, so don't run it in a mode that strips MCP tools.
