...
Prefer no new test over a bad test. A bad test is one that mostly tests mocks, encodes current implementation details, depends on timing or unrelated global state, needs expensive infrastructure for a small fix, or would be deleted immediately after proving the fix. The **better-tests** skill owns the full gate for whether a test is worth writing and the quality rules the test must meet once the failing test exists.
